...
class Autoencoder:
    def __init__(self, device, ngpu = None):
        ...
        model = VQModelInterface(DEFAULT_AE_CONFIG["embed_dim"], DEFAULT_AE_CONFIG["ddconfig"], DEFAULT_AE_CONFIG["lossconfig"], DEFAULT_AE_CONFIG["n_embed"], ckpt_path=f"{get_cache_dir()}/{CACHE_MODEL_DIR}/model.ckpt")
        # model = model.half()
